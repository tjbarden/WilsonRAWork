...
/*gen dist_bins = ceil(distance)
collapse (mean) ssi_rate, by(dist_bins high)
gen mod = mod(dist_bins, 5)
order mod
gen dist_bins5 = dist_bins
replace dist_bins5 = dist_bins + (5-mod) if mod !=0

collapse (mean) ssi_rate, by(dist_bins5 high)
*/


//Bin Scatter
twoway (scatter ssi_rate dist_bins, mcolor(navy)) ///
	(lfit ssi_rate dist_bins if dist_bins<0, lcolor(black)) ///
	(lfit ssi_rate dist_bins if dist_bins>=0, lcolor(black))
...
twoway (scatter ssi_rate dist_bins [aw = numcty], mcolor(navy)) ///
	(lfitci ssi_rate dist_bins [aw = numcty] if dist_bins<0, lcolor(black)) ///
	(lfitci ssi_rate dist_bins [aw = numcty] if dist_bins>=0, lcolor(black))
...
graph export scatter_lpoly_bin5_floor1_mindist.png, replace
